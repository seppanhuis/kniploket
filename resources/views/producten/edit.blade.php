<x-layouts::app :title="$title">
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl p-4 sm:p-6 lg:p-8">
        <div>
            ...
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Wijzig de gegevens van dit product.</p>
        </div>
        ...
        <form method="POST" action="{{ route('producten.update', $product->Id) }}" class="max-w-3xl rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <div class="grid gap-5 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" id="product_naam" name="product_naam" value="{{ old('product_naam', $product->ProductNaam) }}" required class="block w-full rounded-xl border-zinc-300 bg-white px-4 py-2.5 text-zinc-900 shadow-sm focus:border-zinc-900 focus:ring-zinc-900 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100">
                </div>

                <div>
                    <label for="ean_code" class="mb-2 block text-sm font-medium text-zinc-700 dark:text-zinc-200">EAN-code</label>
                    <input type="text" id="ean_code" name="ean_code" value="{{ old('ean_code', $product->EANCode) }}" required maxlength="13" inputmode="numeric" pattern="[0-9]*" oninput="this.value = this.value.replace(/[^0-9]/g, '')" class="block w-full rounded-xl border-zinc-300 bg-white px-4 py-2.5 text-zinc-900 shadow-sm focus:border-zinc-900 focus:ring-zinc-900 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100">
                </div>

                <div>
                    <label for="voorraad" class="mb-2 block text-sm font-medium text-zinc-700 dark:text-zinc-200">Voorraad</label>
                    <input type="number" id="voorraad" name="voorraad" value="{{ old('voorraad', $product->Voorraad) }}" required min="0" class="block w-full rounded-xl border-zinc-300 bg-white px-4 py-2.5 text-zinc-900 shadow-sm focus:border-zinc-900 focus:ring-zinc-900 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100">
                </div>

                <div>
                    <label for="minimum_voorraad" class="mb-2 block text-sm font-medium text-zinc-700 dark:text-zinc-200">Minimum voorraad</label>
                    <input type="number" id="minimum_voorraad" name="minimum_voorraad" value="{{ old('minimum_voorraad', $product->MinimumVoorraad) }}" required min="0" class="block w-full rounded-xl border-zinc-300 bg-white px-4 py-2.5 text-zinc-900 shadow-sm focus:border-zinc-900 focus:ring-zinc-900 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100">
                </div>
                ...
                <div>
                    ...
                    <select id="leverancier_id" name="leverancier_id" required class="block w-full rounded-xl border-zinc-300 bg-white px-4 py-2.5 text-zinc-900 shadow-sm focus:border-zinc-900 focus:ring-zinc-900 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100">
                        @foreach ($suppliers as $supplier)
                            <option value="{{ $supplier->Id }}" {{ old('leverancier_id', $product->LeverancierId) == $supplier->Id ? 'selected' : '' }}>{{ $supplier->Naam }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="categorie_id" name="categorie_id" required class="block w-full rounded-xl border-zinc-300 bg-white px-4 py-2.5 text-zinc-900 shadow-sm focus:border-zinc-900 focus:ring-zinc-900 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100">
                        @foreach ($categories as $category)
                            <option value="{{ $category->Id }}" {{ old('categorie_id', $product->CategorieId) == $category->Id ? 'selected' : '' }}>{{ $category->Naam }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="is_actief" name="is_actief" required class="block w-full rounded-xl border-zinc-300 bg-white px-4 py-2.5 text-zinc-900 shadow-sm focus:border-zinc-900 focus:ring-zinc-900 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100">
                        <option value="1" {{ old('is_actief', $product->IsActief) ? 'selected' : '' }}>Ja</option>
                        <option value="0" {{ old('is_actief', $product->IsActief) == 0 ? 'selected' : '' }}>Nee</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="opmerking" name="opmerking" value="{{ old('opmerking', $product->Opmerking) }}" class="block w-full rounded-xl border-zinc-300 bg-white px-4 py-2.5 text-zinc-900 shadow-sm focus:border-zinc-900 focus:ring-zinc-900 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100">
                </div>
            </div>
